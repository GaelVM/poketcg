...
async function get(name){
  const r=await fetch(BASE+name,{headers:{'user-agent':'poketcg-api-sync/2.0'}});
  if(!r.ok) throw new Error(`${name}: HTTP ${r.status}`);
  return r.json();
}

const [cardsRaw,setsObj,rarities]=await Promise.all([
...
const cards=cardsRaw.map(c=>{
  ...
  return {
    ...
    goodWith:c.goodWith??[],
    packs:c.packs??s.packs??[],
    ex:['ex','mega'].includes(String(c.stage||'').toLowerCase()),
    image:c.image??null,
    // Ruta logica. El frontend/cPanel la resuelve contra su cache local.
    imagePath:(c.set && Number.isFinite(number)) ? `cards/${c.set}/${c.number}.webp` : null,
    setName:s.name?.es || s.name?.en || c.set,
